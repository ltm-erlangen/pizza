...
import subprocess
import sys
from math import atan, cos, fabs, sin
...
class raster:
    ...
    def show(self, ntime):
        ...
        print(output)
        nums = re.findall(r"translation to:\s*(\S*)\s*(\S*)\s*(\S*)\s", output)
        ...
        if len(list) <= 1:
            ...
            while 1:
                ...
                if n == nstart or self.panflag:
                    ...
                    nums = re.findall(
                        r"translation to:\s*(\S*)\s*(\S*)\s*(\S*)\s", output
                    )
                    self.xtrans = float(nums[0][0])
                    self.ytrans = float(nums[0][1])
                    self.ztrans = float(nums[0][2])
    ...
    def single(self, flag, file, box, atoms, bonds, tris, lines):
        ...
        for label in self.labels:
            ...
            print(11, file=f)
            print(
                label[0], label[1], 0.0, label[5][0], label[5][1], label[5][2], file=f
            )
