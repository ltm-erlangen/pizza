...
class gl:
    ...
    def create_window(self):
        ...
        self.w = MyOpengl(
            self.root, width=self.xpixels, height=self.ypixels, double=1, depth=1
        )
    ...
    def mouse_rotate(self, xnew, ynew, xold, yold):
        ...
        if self.ztheta == 0.0:
            ...
            self.azphi = acos(self.view[0] / sin(pi * self.ztheta / 180.0)) / pi * 180.0
        if self.view[1] < 0:
    ...
    def all(self, *list):
        ...
        if len(list) <= 1:
            ...
            while 1:
                ...
                if self.panflag:
                    self.ztheta = self.ztheta_start + fraction * (
                        self.ztheta_stop - self.ztheta_start
                    )
                    self.azphi = self.azphi_start + fraction * (
                        self.azphi_stop - self.azphi_start
                    )
                    self.scale = self.scale_start + fraction * (
                        self.scale_stop - self.scale_start
                    )
                    self.viewupright()
            ...
            if not self.clipflag:
                ...
                if self.bonddraw:
                    ...
                    for bond in self.bonddraw:
                        ...
                        glMaterialfv(
                            GL_FRONT_AND_BACK, GL_EMISSION, [red, green, blue, 1.0]
                        )
    ...
    def save(self, file=None):
        ...
        pstring = glReadPixels(
            0, 0, self.xpixels, self.ypixels, GL_RGBA, GL_UNSIGNED_BYTE
        )
