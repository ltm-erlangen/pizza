...
class patch:
    ...
    def build(self, n, style, *types):
        ...
        for i in range(n):
            exec(cmd)
            self.molecules.append([atoms, bonds, tris, segments, bodies])  # noqa: F821
            self.volume += volume  # noqa: F821
    ...
    def write3d(self, file):
        ...
        if self.lattice[0] or self.lattice[1] or self.lattice[2]:
            latflag = 1
            if self.lattice[0] * self.lattice[1] * self.lattice[2] != len(
                self.molecules
            ):
        ...
        while self.molecules:
            ...
            for bond in molecule[1]:
                idbond += 1
                bonds.append(
                    [idbond, bond[0], bond[1] + idatom + 1, bond[2] + idatom + 1]
                )
            ...
            if self.style == "molecular":
                ...
                for i, atom in enumerate(molecule[0]):
                    ...
                    if not triples:
                        ...
                        triflag = 1
                    atoms.append(
                        [idatom, idmol, atom[0], triflag, mass, x, y, z, ix, iy, iz]
                    )
                    mols.append((idatom, idmol))
                    maxtypes = max(maxtypes, atom[0])
            ...
            if self.style == "molecular":
                ...
                for i, atom in enumerate(molecule[0]):
                    ...
                    if lineflag:
                        ...
                        segment[0], segment[1], tmp = self.pbc_near(
                            segment[0], segment[1], 0, x, y, z
                        )
                        xseg = xnew + segment[2]
                        yseg = ynew + segment[3]
                        segment[2] = xorig + xseg * xp[0] + yseg * yp[0]
                        segment[3] = yorig + xseg * xp[1] + yseg * yp[1]
                        segment[2], segment[3], tmp = self.pbc_near(
                            segment[2], segment[3], 0, x, y, z
                        )
    ...
    def c60hex2(self, *params):
        ...
        diam = params[0]
        patches = [
            [params[2], [1, 2, 3, 4, 5, 6, 34, 35, 46, 47, 48, 60]],
            [params[3], [61]],
        ]
    ...
    def ring(self, *params):
        ...
        for i in range(nring):
            atoms.append(
                [
                    type2,
                    0.5 * diam * cos(i * 2 * pi / nring),
                    0.5 * diam * sin(i * 2 * pi / nring),
                    0.0,
                ]
            )
    ...
    def rod(self, *params):
        ...
        if m2:
            atoms += tether(
                m2, m2type, self.blen, self.dmin, atoms[1], atoms[0], self.random
            )
    ...
    def tri(self, *params):
        ...
        for i in range(nsize):
            ...
            for j in range(n):
                x, y, z = (
                    0.5 * i * self.blen + j * self.blen,
                    i * self.blen * sqrt(3.0) / 2.0,
                    0.0,
                )
                atoms.append([ntype, x, y, z])
        ...
        if m1:
            atoms += tether(
                m1, m1type, self.blen, self.dmin, atoms[1], atoms[0], self.random
            )
        if m2:
            atoms += tether(
                m2,
    ...
    def hex(self, *params):
        ...
        if m1:
            atoms += tether(
                m1, m1type, self.blen, self.dmin, atoms[0], atoms[1], self.random
            )
        if m2:
            atoms += tether(
                m2, m2type, self.blen, self.dmin, atoms[0], atoms[2], self.random
            )
        if m3:
            atoms += tether(
                m3, m3type, self.blen, self.dmin, atoms[0], atoms[3], self.random
            )
        if m4:
            atoms += tether(
                m4, m4type, self.blen, self.dmin, atoms[0], atoms[4], self.random
            )
        if m5:
            atoms += tether(
                m5, m5type, self.blen, self.dmin, atoms[0], atoms[5], self.random
            )
        if m6:
            atoms += tether(
                m6, m6type, self.blen, self.dmin, atoms[0], atoms[6], self.random
            )
        ...
        for i in range(m5):
            if i == 0:
                ...
                bonds.append(
                    [1, n + m1 + m2 + m3 + m4 + i - 1, n + m1 + m2 + m3 + m4 + i]
                )
        for i in range(m6):
            if i == 0:
