...
class sdata:
    ...
    def read(self, id, *list):
            ...
            while 1:
                ...
                if "points" in line:
                    ...
                    for i in range(npoints):
                        ...
                        if len(pt) == 2:
                            pt.append(0.0)  # treat pts as 3-vecs
                        points.append(pt)
                    ...
                    for i in range(nlines):
                        ...
                        lines.append(
                            [
                                int(value) - npoints_prev - 1
                                for value in words[1 + nflags :]
                            ]
                        )
        ...
        if self.dim == 2:
            print(
                "read surf %s with %d points, %d lines" % (id, len(points), len(lines))
            )
        if self.dim == 3:
            print(
                "read surf %s with %d points, %d triangles"
    ...
    def write(self, file, pflag=1, nfile=0, ids=[]):
        ...
        if nfile == 0:
            self.write_single(
                file, pflag, points, lines, lineflags, triangles, triflags
            )
        else:
            self.write_parallel(
                file, nfile, points, lines, lineflags, triangles, triflags
            )

        if pflag:
            if self.dim == 2:
                print(
                    "wrote surf file with %d points, %d lines"
                    % (len(points), len(lines))
                )
            if self.dim == 3:
                print(
                    "wrote surf file with %d points, %d triangles"
                    % (len(points), len(triangles))
                )
        else:
            if self.dim == 2:
                print("wrote surf file with %d lines" % len(lines))
            if self.dim == 3:
                print("wrote surf file with %d triangles" % len(triangles))
    ...
    def write_parallel(
        self, file, nfile, points, lines, lineflags, triangles, triflags
    ):
        basefile = file + ".base"
        fp = open(basefile, "w")
        ...
        for ifile in range(nfile):
            ...
            print(file=fp)
            if self.dim == 2:
                print(istop - istart, "lines", file=fp)
            if self.dim == 3:
                print(istop - istart, "triangles", file=fp)
            print(file=fp)
    ...
    def gridfile(self, *args):
        if self.mode != "SPARTA":
            raise Exception("gridfile() can only be used with SPARTA mode")
        if self.dim == 0:
        ...
        if self.gridflag and self.dim == 2:
            for idparent in self.idparents:
                ...
                if not box:
                    ...
                    iy = (idchild - 1) / nx
                    box = (
                        xlo + float(ix) * (xhi - xlo) / nx,
                        xlo + float(ix + 1) * (xhi - xlo) / nx,
                        ylo + float(iy) * (yhi - ylo) / ny,
                        ylo + float(iy + 1) * (yhi - ylo) / ny,
                    )
                ...
                for i in range(nx + 1):
                    ...
                    lines.append(
                        [id + i, itype + nlevel + 1] + [x, ylo, 0.0, x, yhi, 0.0]
                    )
                id += nx + 1
                for i in range(ny + 1):
                    y = ylo + float(i) * (yhi - ylo) / ny
                    lines.append(
                        [id + i, itype + nlevel + 1] + [xlo, y, 0.0, xhi, y, 0.0]
                    )
                id += ny + 1
            ...
            for idparent in self.idparents:
                ...
                if not box:
                    ...
                    box = (
                        ...
                        ylo + float(iy) * (yhi - ylo) / ny,
                        ylo + float(iy + 1) * (yhi - ylo) / ny,
                        zlo + float(iz) * (zhi - zlo) / nz,
                        zlo + float(iz + 1) * (zhi - zlo) / nz,
                    )
                    self.parents[idparent] = [box, subgrid]
                ...
                for i in range(ny + 1):
                    ...
                    for j in range(nz + 1):
                        ...
                        lines.append(
                            [id + i, itype + nlevel + 1] + [xlo, y, z, xhi, y, z]
                        )
                id += (ny + 1) * (nz + 1)
                for i in range(nx + 1):
                    x = xlo + float(i) * (xhi - xlo) / nx
                    for j in range(nz + 1):
                        z = zlo + float(j) * (zhi - zlo) / nz
                        lines.append(
                            [id + i, itype + nlevel + 1] + [x, ylo, z, x, yhi, z]
                        )
                id += (nx + 1) * (nz + 1)
                for i in range(nx + 1):
                    x = xlo + float(i) * (xhi - xlo) / nx
                    for j in range(ny + 1):
                        ...
                        lines.append(
                            [id + i, itype + nlevel + 1] + [x, y, zlo, x, y, zhi]
                        )
                id += (nx + 1) * (ny + 1)
...
def connect_triangles(tris):
    ...
    for i, tri in enumerate(tris):
        entry = []
        reverse = (tri[1], tri[0])
        if reverse not in hash:
            entry.append((-1, 0))
        else:
            entry.append(hash[reverse])
        reverse = (tri[2], tri[1])
        if reverse not in hash:
            entry.append((-1, 0))
        else:
